...
-  [XAIPostProcessor versión normalizada](scripts/XAIPostProcessor_norm.py): Igual que el caso anterior con la salvedad de que se normaliza cada variable al intervalo [0,1] para que los valores de las features no sean demasiado superiores a los de CRP. El proceso de normalización es el siguiente. Supongamos que el vector $[feat(x),CRP(x)]$ tiene dimensión $D \forall x \in ID \cup OOD$. Supongamos ahora una matriz que contiene estos vectores en filas es decir, una matriz de dimensión $(Num\_{muestras}, D)$. Entonces cada columna se divide por el máximo en valor absoluto de todos los valores de la columna.
