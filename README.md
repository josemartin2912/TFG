...
-  [MahaPostProcessor](scripts/MahaPostProcessor.py): Se calcula la distribución $(\mu,\Sigma)$ de las features de ejemplos ID. Se devuelve como score ID la distancia de Mahalanobis a la distribución, es decir, $\forall x \in ID \quad d(x) = (x-\mu)^T\Sigma^{-1}(x-\mu)$. Se devuelve este valor en negativo por una razón. OpenOOD asigna como ID a los scores más altos. Por tanto, como calculamos una distancia, menos distancia implica mayor probabilidad ID. Entonces mayor **-distancia** implica mayor probabilidad de ID.
